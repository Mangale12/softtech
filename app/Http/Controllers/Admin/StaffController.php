...
class StaffController extends DM_BaseController
{
    ...
    public function store(Request $request)
    {
        // dd($request->all());
        $request->validate([
            //'country_member'        => 'required|max:225',
            'name'                  => 'required|max:225',
            'designation'           => 'required|max:225',
            'image'                 => 'required|mimes:jpeg,jpg,png,gif|max:50000',
            'status'                => 'required|boolean'
        ]);

        if ($this->model->storeData($request, $request->country_member, $request->name, $request->designation, $request->description, $request->facebook, $request->twitter, $request->insta, $request->image, $request->status)) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($this->model->updateData($request, $id, $request->country_member, $request->name, $request->designation, $request->description, $request->facebook, $request->twitter, $request->insta, $request->image, $request->status)) {
